...
async def tasks_by_user_id(db2: Annotated[Session, Depends(get_db)],user_id: int):
    user = db2.scalar(select(User).where(User.id == user_id))
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    tasks = db2.scalars(select(Task).where(Task.user_id == user_id)).all()
    return tasks
...
async def update_user(db2: Annotated[Session, Depends(get_db)], user_id: int, update_user: CreateUser):
    ...
    if update_user.username == "string":
        update_user.username=user.username
    if update_user.firstname == "string":
        update_user.firstname = user.firstname
    if update_user.lastname == "string":
        update_user.lastname = user.lastname
    if update_user.age == 0:
...
async def delete_user(db2: Annotated[Session, Depends(get_db)], user_id: int):
    ...
    db2.execute(delete(Task).where(Task.user_id == user_id))
    db2.execute(delete(User).where(User.id == user_id))
    db2.commit()
    return {
        'status_code': status.HTTP_200_OK,
